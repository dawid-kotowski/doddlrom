...
stat_Coeff_Trainer = rom.statHadamardNNTrainer(
    stat_DOD_model, stat_Coeff_model, P.N,
    stat_train_valid_data,
    trainer["epochs"], trainer["restarts"],
    learning_rate=1e-3, batch_size=128, patience=trainer["patience"]
)
...
torch.save(stat_DOD_model.state_dict(),   state_dict_path / 'stat_DOD_Module.pth')
torch.save(stat_Coeff_model.state_dict(), state_dict_path / 'stat_CoeffDOD_Module.pth')
torch.save(CoLoRA_DL_model.state_dict(),     state_dict_path / 'CoLoRA_Module.pth')